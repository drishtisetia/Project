...
@Override
public List<ProductDTO> getProductBySellerEmailId(String sellerEmailId) throws EKartException {
	
	List<Product> products = productRepository.findBySellerEmailId(sellerEmailId);
	if(products.isEmpty()) {
		throw new EKartException("DealsService.INVALID_SELLER_EMAIL");
	}else {
		...
		p1.setSellerEmailId(p.getSellerEmailId());
		productsDTO.add(p1);
		}
		return productsDTO;
	}
